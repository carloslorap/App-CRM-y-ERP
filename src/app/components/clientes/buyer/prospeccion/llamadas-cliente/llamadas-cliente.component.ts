...
export class LlamadasClienteComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._route.params.subscribe((params) => {
      this.id = params['id'];
      this._clienteService.obtener_datos_cliente_admin(this.id,this.token).subscribe(response=>{
  ...
  init_data(){
    this._clienteService.listar_llamadas_prospeccion_admin(this.id,this.token).subscribe(response=>{
  ...
  registrar() {
    ...
    if (!this.llamada.fecha) {
      ...
      this._clienteService
        .crear_llamada_prospeccion_admin(this.llamada, this.token)
        .subscribe((response) => {
