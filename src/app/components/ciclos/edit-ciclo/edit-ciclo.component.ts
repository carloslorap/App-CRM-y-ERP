...
export class EditCicloComponent implements OnInit {
  ...
  public docente_salones :Array<any> = [];
  ...
  ngOnInit(): void {
    this._route.params.subscribe((params) => {
      ...
      this._cursoService
        .obtener_datos_curso_ciclo_admin(this.id, this.idciclo, this.token)
        .subscribe((response) => {
  ...
  agregar_ciclo() {
    ...
    if (!this.salon.salon) {
      ...
      this._cursoService
        .agregar_salon_ciclo_admin(this.salon, this.token)
        .subscribe((response) => {
          this.dias = [];
          this.salon = { salon: '' };
  ...
  init_salones() {
    this._cursoService
      .obtener_salones_ciclo_admin(this.id, this.token)
      .subscribe((response) => {
  ...
  init_docentes() {
    this._colaboradorService
      .listar_docentes_admin(this.token)
      .subscribe((response) => {
  ...
  init_docentes_salones() {
    this._cursoService
      .listar_docentes_salon_admin(this.idciclo,this.token)
      .subscribe((response) => {
        this.docente_salones = response.data;
        console.log(this.docente_salones);
  ...
  eliminar_salon(idx: any) {
    ...
    this._cursoService
      .eliminar_salon_ciclo_admin(idx, this.token)
      .subscribe((response) => {
  ...
  Actualizar() {
    if (!this.ciclo.nivel) {
      ...
      this._cursoService
        .editar_ciclo_admin(this.idciclo, this.ciclo, this.token)
        .subscribe((response) => {
          $.notify('Se actualizo el ciclo', {
            type: 'success',
            spacing: 10,
            timer: 2000,
            placement: {
  ...
  filtrar_docente(){
    if (this.filtro_docente) {
      ...
      this.docentes = this.docentes_const.filter(item =>term.test(item.nombres) || term.test(item.apellidos))
    } else {
  ...
  vincular_docente_salon(){
    ...
    if (!this.docente_salon.colaborador) {
      ...
      this._cursoService.agregar_docente_salon_admin(this.docente_salon,this.token).subscribe(response=>{
        $.notify('Se Agrego correctamente la vinculacion', {
          type: 'success',
          spacing: 10,
          timer: 2000,
          placement: {
  ...
  eliminar_docente_salon(id:any){
    ...
    this._cursoService
      .eliminar_docente_salon_admin(id, this.token)
      .subscribe((response) => {
