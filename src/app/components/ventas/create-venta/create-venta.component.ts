...
export class CreateVentaComponent implements OnInit {
  ...
  constructor(private _clienteService: ClienteService,private _ventaService:VentaService,private _router:Router,private _testService:TestService){
    this.init_config()
  }
  ...
  init_variedades(){
    ...
    this.variedades = [];
    this._ventaService.obtener_variedades_admin(this.token).subscribe(response=>{
  ...
  ingresar_venta(){
    ...
    if(!this.venta.cliente){
      ...
      this.load_btn = true
      this._ventaService.generar_venta_admin(this.venta,this.token).subscribe(response=>{
